...
- **`POST /token`** — OAuth token exchange: the broker holds the durable
  ...
  **Identity → tenant / profile id**: emails map to a stable hash shared with
  calsync (`i` + the first 32 hex characters of SHA-256 of the lowercased
  address). That value is calsync's auto tenant id and the gateway's per-user
  profile directory name. Distinct addresses that only differ in punctuation
  no longer collide. Pre-hash profile directories (`owner_at_example_com`) are
  still read as a migration fallback. Existing calsync punctuation-slug
  tenants and their broker slots (`<slug>_personal`) need
